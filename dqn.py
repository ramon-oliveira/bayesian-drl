...
def populate_memory(env, D, m, k, max_steps):
    state0_rm, action_rm, reward_rm, state1_rm = D
...
if __name__ == '__main__':
    ...
    C = 5000 # frequency target update
    render = False
    ...
    D = [state0_rm, action_rm, reward_rm, state1_rm]
    populate_memory(env, D, m, k, max_steps)

    # batch array
    state0_batch = np.zeros([replay_size, m, size, size], dtype=np.int8)
    action_batch = np.zeros([replay_size], dtype=np.int8)
    reward_batch = np.zeros([replay_size], dtype=np.int8)
    state1_batch = np.zeros([replay_size, m, size, size], dtype=np.int8)
        ...
        for t in range(max_steps):
            if (t % k) == 0:
                if np.random.rand() < e:
                    ...
                    qval = Q.predict(np.array([obs0]), batch_size=1, verbose=0)
                    action = qval.argmax()
            (ob, reward, done, _info) = env.step(action)
            ...
            if (t % k) == 0:
                ...
                y_Q = Q.predict_on_batch(state0_rm[idxs])
                y_Q_target = reward_rm[idxs] + gama*np.max(Q_target.predict(state1_rm[idxs]), axis=1)
